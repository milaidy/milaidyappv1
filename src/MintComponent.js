...
function MintComponent({ imageUri, ipfsLink }) {
  const [tokenId, setTokenId] = useState('');
  const [minting, setMinting] = useState(false);

  const contractAddress = '0x3E8Ea79dcbd42Ff525bd98A56Cc0b63f58f2eBbb'; // Set the contract address here
  ...
  const mintNFT = async () => {
    ...
    try {
      ...
      ];  // You have not provided the ABI here
      ...
      const ipfsImageUri = imageUri;
      console.log("Minting with IPFS URI:", ipfsImageUri);

      // Call the safeMinconsole.log("Minting with IPFS URI:", ipfsImageUri);
      const result = await contract.methods.safeMint(accounts[0], ipfsImageUri).send({
        value: payableAmount,
        from: accounts[0] // Use the first account as the "from" address
      });
  ...
  return (
    <div>
      ...
      {tokenId && !minting && (
        <div>
          <h4>YOU JUST MINTED Token ID: {tokenId}</h4>
          <h4>!DO NOT MINT AGAIN UNLESS YOU WANT A DUPLICATE TOKEN!</h4>
          <h4>refresh the page to gen. new img (:</h4>
